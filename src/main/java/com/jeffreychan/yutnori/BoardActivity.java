...
public class BoardActivity extends Activity implements OnClickListener{
	...
	ImageView[] rollSlot, tiles, playerOneChars, playerTwoChars;

	LinearLayout topBar, bottomBar;
	RelativeLayout rl;

	boolean isRollDone, canRoll = true, isEndTurn, moveDone, capture;
	...
	public void onClick(View v) {
		...
		if (moveDone) {
			...
			if (!capture) {
				...
				if (counter == 0 && isRollDone) endTurn();
			} else {
	...
	private void showPossibleTiles(int pi){
		...
		if (turn == 0) p1current = playerOneChars[pi];
		else p2current = playerTwoChars[pi];
